...
d3 %>% group_by(gene_id) %>% count() %>% head()
...
d7 %>% group_by(chrom) %>% filter(!chrom %in% c("chrX", "chrY")) %>% count() %>% pull(n) %>% median()

#Mean
d7 %>% group_by(chrom) %>% filter(!chrom %in% c("chrX", "chrY")) %>% count() %>% pull(n) %>% mean()
