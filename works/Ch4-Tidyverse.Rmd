...
```{r}
answer1 <- "d. co2 is not tidy: to be tidy we would have to wrangle it to have three columns (year, month and value), then each co2 observation would have a row."
answer1
```
...
```{r}
answer2 <- "b. ChickWeight is tidy: each observation (a weight) is represented by one row. The chick from which this measurement came is one of the variables."
answer2
```
...
```{r}
answer3 <- "c. BOD is tidy: each row is an observation with two values (time and demand)"
answer3
```
...
```{r}
answer <- "b. EuStockMarkets, c. DNase, d. Formaldehyde, e. Orange"
```
...
my_states %>% pander()
#Be careful! You should omit the first argument when you use a pipe.
```
