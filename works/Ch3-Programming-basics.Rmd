...
This table helps us see what happened:(table)
...
1+2+...+n is n(n+1)/2. What if we weren’t sure that was the right function? How could we check? Using what we learned about functions we can create one that computes the $S_n$:
```{r}
...
In each iteration  n=1 ,  n=2 , etc…, we compute  
$S_n$ and store it in the  nth entry of $S_n$.
...
```
(There is a '-3' in the vector and not all entries are bigger than 0, so it will proceed to 'else' part)
...
answer
```
This is because at least one entry will be FALSE if '!' is applied.
...
```
The value of x is 3, because variables defined inside a function are not saved in the workspace.
...
# Be careful!
#'a' should be written 'inside' function(n), or a will just stay as a fixed vector. It will not change along with what's happening inside the 'compute_s_n', or other functions that use compute_s_n.
...
```{r}
#The function format : map_dbl(.x, .f, ...) 
#'.x' is a vector, '.f' is a function. We execute '.f' to each of the entries in '.x'.
library(purrr)
purrr::map_dbl
...
#In this case, both arguments are 'vectors.'
...
#Other methods : sapply, map_dbl
n <- 1:25

identical(sapply(n, compute_s_n),n*(n+1)*(2*n+1)/6)

identical(map_dbl(n, compute_s_n), n*(n+1)*(2*n+1)/6)
```
