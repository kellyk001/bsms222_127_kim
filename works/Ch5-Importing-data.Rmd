...
```{r}
file.copy(fullpath, "murders-test.csv")
#> [1] TRUE
```
...
dir <- system.file("extdata", package = "dslabs") 
fullpath <- file.path(dir, filename)
file.copy(fullpath, "murders.csv") #FALSE, because the file is already been copied in the chunks above.
```
...
library(pander)
setwd(dir)
read_csv("olive.csv") %>% head() %>% pander()
